...
Use the root model as the orchestrator. Optimize for correct results with minimal expensive-model usage.
...
Action: root handles directly, or `fast_worker` only if delegation clearly saves context. No mandatory explorer/reviewer.

### M — normal engineering
Signs: bounded feature/bug, a few related files, architecture is clear, moderate contained risk.
Action: use `explorer` only if locations/flow are unknown; implementation defaults to `worker`; use `tester` after meaningful changes; reviewer only when justified.
...
Action: parallel `explorer`/`researcher`; root decides architecture; `worker` for normal slices; `expert` only for hard slices; `tester`; finish with `reviewer`.
...
Action: root owns architecture; `expert` for difficult implementation; `tester`; `reviewer`; `deep_reviewer` only when an independent Astra pass is worth the allowance.

## Step 2 — parallelism
Parallelize only independent work. Good: backend discovery + frontend discovery; docs research + repo exploration; workers on non-overlapping files. Avoid two workers editing the same contract/file. Do not spawn roles mechanically. Target 2–4 active subagents.
...
Terra -> Sol when root cause is subtle, algorithms/concurrency/performance matter, architecture tradeoffs appear, or repeated attempts fail.
Sol -> Astra deep review only for high blast radius, expensive/irreversible failure, or a materially useful independent top-tier judgment.
